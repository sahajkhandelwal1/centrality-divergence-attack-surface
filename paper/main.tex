...
Figure~\ref{fig:network_viz} shows representative network visualizations with
...
Figure~\ref{fig:rank_scatter} shows BC rank vs.\ EC rank scatter plots for
...
Figure~\ref{fig:attack_curves} shows representative attack curves $S(f)$ and
...
\begin{figure*}[!t]
  ...
  \caption{Attack curves $S(f)$ (giant fraction) and $\chi(f)$ (susceptibility)
           ...
           Left pair per model: high connectivity; right pair: low connectivity
           (where EC degrades).
...
Figure~\ref{fig:main_scatter} shows the main result: all 23 conditions have
...
Figure~\ref{fig:tau_traj} shows $\tau_{\rm BC}(f)$ (Kendall $\tau$ between BC
...
Figure~\ref{fig:tau_rate} shows the derivative
...
Figure~\ref{fig:fss} shows $f_c$ vs.\ $N$ for both attackers.
